classdef Transpose_To_UnsqueezeLayer1037 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Transpose_To_UnsqueezeLayer1037(name, onnxParams)
            this.Name = name;
        ...
        function [x_conv_layers_0_Unsq] = predict(this, input)
            ...
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x_conv_layers_0_Unsq}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'Transpose_To_UnsqueezeLayer1037');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Transpose_To_UnsqueezeLayer1037'));
            end
...
[x_conv_layers_0_Unsq, x_conv_layers_0_UnsqNumDims, state] = Transpose_To_UnsqueeGraph1032(input, NumDims.input, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x_conv_layers_0_Unsq] = postprocessOutput(x_conv_layers_0_Unsq, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x_conv_layers_0_Unsq, x_conv_layers_0_UnsqNumDims1036, state] = Transpose_To_UnsqueeGraph1032(input, inputNumDims1035, Vars, NumDims, Training, state)
% Function implementing the graph 'Transpose_To_UnsqueeGraph1032'
...
% Transpose:
[perm, NumDims.x_Transpose_output_0] = prepareTransposeArgs(Vars.TransposePerm1033, NumDims.input);
if ~isempty(perm)
    Vars.x_Transpose_output_0 = permute(Vars.input, perm);
end

% Unsqueeze:
[shape, NumDims.x_conv_layers_0_Unsq] = prepareUnsqueezeArgs(Vars.x_Transpose_output_0, Vars.UnsqueezeAxes1034, NumDims.x_Transpose_output_0);
Vars.x_conv_layers_0_Unsq = reshape(Vars.x_Transpose_output_0, shape);

% Set graph output arguments from Vars and NumDims:
x_conv_layers_0_Unsq = Vars.x_conv_layers_0_Unsq;
x_conv_layers_0_UnsqNumDims1036 = NumDims.x_conv_layers_0_Unsq;
