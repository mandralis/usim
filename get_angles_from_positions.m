...
Pkin = forward_kin(0,a,a,a,a,0,0,0,0);
...
%     query_x = [1,query_x];
    ...
    Theta_relative(i,1) = Theta(i,1);
    Theta_relative(i,2) = Theta(i,2) - Theta(i,1);
    Theta_relative(i,3) = Theta(i,3) - Theta(i,2);
    Theta_relative(i,4) = Theta(i,4) - Theta(i,3);

%     % plot kinematic linkage with given angles
%     plot(Px(i,:),Py(i,:),"Color",'b',LineWidth=1.0);
%     axis([0,L,0,1080])
%     hold on
%     Pkin = forward_kin(0,a,a,a,a,Theta(i,1),Theta(i,2) - Theta(i,1),Theta(i,3) - Theta(i,2),Theta(i,4) - Theta(i,3));
%     plot(Pkin(2,:),Pkin(1,:) + py_clamp,'Marker','o','MarkerFaceColor','k',"Color",'r','MarkerEdgeColor','k',LineWidth=1.0);
%     pause(0.01);
%     clf;
end
