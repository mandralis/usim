classdef Slice_To_UnsqueezeLayer1043 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Slice_To_UnsqueezeLayer1043(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'x_conv1_Unsqueeze_ou'};
            this.ONNXParams = onnxParams;
        end
        
        function [x_conv1_Unsqueeze_ou] = predict(this, input)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = 2;
            onnxParams = this.ONNXParams;
            [x_conv1_Unsqueeze_ou, x_conv1_Unsqueeze_ouNumDims] = Slice_To_UnsqueezeFcn(input, inputNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[2 1], ['as-is']}, ...
                'OutputDataPermutation', {[2 1 3], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x_conv1_Unsqueeze_ou}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'Slice_To_UnsqueezeLayer1043');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Slice_To_UnsqueezeLayer1043'));
            end
            x_conv1_Unsqueeze_ou = dlarray(single(x_conv1_Unsqueeze_ou), 'CBT');
            if ~coder.target('MATLAB')
                x_conv1_Unsqueeze_ou = extractdata(x_conv1_Unsqueeze_ou);
            end
        end
        
        function [x_conv1_Unsqueeze_ou] = forward(this, input)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = 2;
            onnxParams = this.ONNXParams;
            [x_conv1_Unsqueeze_ou, x_conv1_Unsqueeze_ouNumDims] = Slice_To_UnsqueezeFcn(input, inputNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[2 1], ['as-is']}, ...
                'OutputDataPermutation', {[2 1 3], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x_conv1_Unsqueeze_ou}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'Slice_To_UnsqueezeLayer1043');
...
function [x_conv1_Unsqueeze_ou, x_conv1_Unsqueeze_ouNumDims, state] = Slice_To_UnsqueezeFcn(input, inputNumDims, params, varargin)
%SLICE_TO_UNSQUEEZEFCN Function implementing an imported ONNX network.
%
...
%
% [X_CONV1_UNSQUEEZE_OU] = Slice_To_UnsqueezeFcn(INPUT, PARAMS)
%			- Evaluates the imported ONNX network SLICE_TO_UNSQUEEZEFCN with input(s)
%			INPUT and the imported network parameters in PARAMS. Returns
%			network output(s) in X_CONV1_UNSQUEEZE_OU.
%
% [X_CONV1_UNSQUEEZE_OU, STATE] = Slice_To_UnsqueezeFcn(INPUT, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = Slice_To_UnsqueezeFcn(INPUT, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
% Outputs:
% --------
% X_CONV1_UNSQUEEZE_OU
%			- Output(s) of the ONNX network.
%			  Without permutation, the size(s) of the outputs are:
%				  X_CONV1_UNSQUEEZE_OU:		[Unknown, Unknown, Unknown]				Type: FLOAT
...
[x_conv1_Unsqueeze_ou, x_conv1_Unsqueeze_ouNumDims, state] = Slice_To_UnsqueezeGraph1038(input, NumDims.input, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x_conv1_Unsqueeze_ou] = postprocessOutput(x_conv1_Unsqueeze_ou, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x_conv1_Unsqueeze_ou, x_conv1_Unsqueeze_ouNumDims1042, state] = Slice_To_UnsqueezeGraph1038(input, inputNumDims1041, Vars, NumDims, Training, state)
% Function implementing the graph 'Slice_To_UnsqueezeGraph1038'
...
% Slice:
[Indices, NumDims.x_Slice_output_0] = prepareSliceArgs(Vars.input, Vars.x_Constant_1_output_, Vars.x_Constant_2_output_, Vars.x_Constant_output_0, Vars.x_Constant_3_output_, NumDims.input);
Vars.x_Slice_output_0 = subsref(Vars.input, Indices);

% Transpose:
[perm, NumDims.x_Transpose_output_0] = prepareTransposeArgs(Vars.TransposePerm1039, NumDims.x_Slice_output_0);
if ~isempty(perm)
    Vars.x_Transpose_output_0 = permute(Vars.x_Slice_output_0, perm);
end

% Unsqueeze:
[shape, NumDims.x_conv1_Unsqueeze_ou] = prepareUnsqueezeArgs(Vars.x_Transpose_output_0, Vars.UnsqueezeAxes1040, NumDims.x_Transpose_output_0);
Vars.x_conv1_Unsqueeze_ou = reshape(Vars.x_Transpose_output_0, shape);

% Set graph output arguments from Vars and NumDims:
x_conv1_Unsqueeze_ou = Vars.x_conv1_Unsqueeze_ou;
x_conv1_Unsqueeze_ouNumDims1042 = NumDims.x_conv1_Unsqueeze_ou;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(input, numDataOutputs, params, varargin)
% Function to validate inputs to Slice_To_UnsqueezeFcn:
p = inputParser;
...
function [x_conv1_Unsqueeze_ou] = postprocessOutput(x_conv1_Unsqueeze_ou, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
if ~anyDlarrayInputs && ~Training
    if isdlarray(x_conv1_Unsqueeze_ou)
        x_conv1_Unsqueeze_ou = extractdata(x_conv1_Unsqueeze_ou);
    end
end
% Permute outputs if requested:
x_conv1_Unsqueeze_ou = permuteOutputVar(x_conv1_Unsqueeze_ou, outputDataPerms{1}, 3);
end
