classdef SliceLayer1037 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = SliceLayer1037(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'x_Slice_1_output_0'};
            this.ONNXParams = onnxParams;
        end
        
        function [x_Slice_1_output_0] = predict(this, input)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = 2;
            onnxParams = this.ONNXParams;
            [x_Slice_1_output_0, x_Slice_1_output_0NumDims] = SliceFcn(input, inputNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[2 1], ['as-is']}, ...
                'OutputDataPermutation', {[2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x_Slice_1_output_0}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1037');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'SliceLayer1037'));
            end
            x_Slice_1_output_0 = dlarray(single(x_Slice_1_output_0), 'CB');
            if ~coder.target('MATLAB')
                x_Slice_1_output_0 = extractdata(x_Slice_1_output_0);
            end
        end
        
        function [x_Slice_1_output_0] = forward(this, input)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = 2;
            onnxParams = this.ONNXParams;
            [x_Slice_1_output_0, x_Slice_1_output_0NumDims] = SliceFcn(input, inputNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[2 1], ['as-is']}, ...
                'OutputDataPermutation', {[2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x_Slice_1_output_0}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1037');
...
function [x_Slice_1_output_0, x_Slice_1_output_0NumDims, state] = SliceFcn(input, inputNumDims, params, varargin)
...
%
% [X_SLICE_1_OUTPUT_0] = SliceFcn(INPUT, PARAMS)
%			- Evaluates the imported ONNX network SLICEFCN with input(s)
%			INPUT and the imported network parameters in PARAMS. Returns
%			network output(s) in X_SLICE_1_OUTPUT_0.
%
% [X_SLICE_1_OUTPUT_0, STATE] = SliceFcn(INPUT, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = SliceFcn(INPUT, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
%				  X_SLICE_1_OUTPUT_0:		[Unknown, Unknown]				Type: FLOAT
...
[x_Slice_1_output_0, x_Slice_1_output_0NumDims, state] = SliceGraph1034(input, NumDims.input, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x_Slice_1_output_0] = postprocessOutput(x_Slice_1_output_0, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x_Slice_1_output_0, x_Slice_1_output_0NumDims1036, state] = SliceGraph1034(input, inputNumDims1035, Vars, NumDims, Training, state)
% Function implementing the graph 'SliceGraph1034'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.input = input;
NumDims.input = inputNumDims1035;

% Execute the operators:
% Slice:
[Indices, NumDims.x_Slice_1_output_0] = prepareSliceArgs(Vars.input, Vars.x_Constant_5_output_, Vars.x_Constant_6_output_, Vars.x_Constant_4_output_, Vars.x_Constant_7_output_, NumDims.input);
Vars.x_Slice_1_output_0 = subsref(Vars.input, Indices);

% Set graph output arguments from Vars and NumDims:
x_Slice_1_output_0 = Vars.x_Slice_1_output_0;
x_Slice_1_output_0NumDims1036 = NumDims.x_Slice_1_output_0;
...
function [x_Slice_1_output_0] = postprocessOutput(x_Slice_1_output_0, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
if ~anyDlarrayInputs && ~Training
    if isdlarray(x_Slice_1_output_0)
        x_Slice_1_output_0 = extractdata(x_Slice_1_output_0);
    end
end
% Permute outputs if requested:
x_Slice_1_output_0 = permuteOutputVar(x_Slice_1_output_0, outputDataPerms{1}, 2);
end
