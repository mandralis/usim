...
class ConvLSTMNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(dim_in,batch_size * seq_length)
...
if __name__ == '__main__':
    ...
    # # reshape data to be in (-1,seq_length,dim_in) shape
    # max_num_rows = (X.size(0) // seq_length) * seq_length
    # X = X[:max_num_rows,:].view(-1, seq_length, dim_in)
    # Theta = Theta[:max_num_rows,:].view(-1, seq_length, dim_out)
    ...
    X_train, X_val, Theta_train, Theta_val = train_test_split(X, Theta, test_size=validation_split, shuffle=False)
